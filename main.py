...
#   db: Annotated[AsyncMongoClient, Depends(get_database)]
# ):
...
async def fetch_film_stat(
    ...
    db: Annotated[AsyncMongoClient, Depends(get_database)] = None
  ):
  try:
    await db.command('ping')
  except Exception:
...
async def parse_list(id: int, db: Annotated[AsyncMongoClient, Depends(get_database)], fetch_movies: bool = False) -> ListHistory:
  try:
      await db.command('ping')
  except Exception:
      raise HTTPException(status_code=500, detail="Database connection error")
